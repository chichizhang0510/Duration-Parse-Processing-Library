...
public class DurationNormalizer {
  ...
  public static NormalizedParts normalize(long totalSeconds) {
    ...
    final int sign = signOf(totalSeconds);
    long absSeconds =
        safeAbs(totalSeconds); // Handle Long.MIN_VALUE safely (Math.abs(Long.MIN_VALUE) overflows)

    final long weeks = absSeconds / SEC_PER_WEEK;
    absSeconds %= SEC_PER_WEEK;

    final long days = absSeconds / SEC_PER_DAY;
    absSeconds %= SEC_PER_DAY;

    final long hours = absSeconds / SEC_PER_HOUR;
    absSeconds %= SEC_PER_HOUR;

    final long minutes = absSeconds / SEC_PER_MIN;
    final long seconds = absSeconds % SEC_PER_MIN;

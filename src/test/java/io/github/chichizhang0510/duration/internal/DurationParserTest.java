...
class DurationParserTest {
  ...
  @ParameterizedTest
  @CsvSource({"-30m, -1800", "-2h30m, -9000", "'-2h 30m', -9000", "-90s, -90"})
  ...
  void parse_invalidInputs_throw(String input) {
    assertThrows(
        InvalidDurationFormatException.class, () -> DurationParser.parseToTotalSeconds(input));
  }

  @Test
  void parse_null_throws() {
    assertThrows(
        InvalidDurationFormatException.class, () -> DurationParser.parseToTotalSeconds(null));
  }

  @Test
  void parse_blank_throws() {
    assertThrows(
        InvalidDurationFormatException.class, () -> DurationParser.parseToTotalSeconds("   "));
  }
  ...
  void parse_wrongOrder_hasHelpfulMessage() {
    InvalidDurationFormatException ex =
        assertThrows(
            InvalidDurationFormatException.class,
  ...
  void parse_duplicateUnit_hasHelpfulMessage() {
    InvalidDurationFormatException ex =
        assertThrows(
            InvalidDurationFormatException.class, () -> DurationParser.parseToTotalSeconds("2h2h"));
  ...
  void parse_overflow_throws() {
    ...
    assertThrows(
        InvalidDurationFormatException.class, () -> DurationParser.parseToTotalSeconds(hugeWeeks));
  }
}
